...
#通过pandas dataframe来存储表情以及输出csv
import pandas as pd
...
def predict_expression():
    ...
    while True:
        ...
        frame = cv2.cvtColor(cv2.resize(frame, (800, 600)), cv2.COLOR_BGR2RGB)#裁剪
        ...
        #将记录下来的表情通过dataframe记录
        emotion_dataframe = pd.DataFrame(recorder)
        emotion_dataframe.to_csv('emotion_recorder.csv')
...
def vid_predict_expression(filename):
    """
    实时预测
    :return:
    """
