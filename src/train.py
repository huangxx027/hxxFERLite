...
if opt.dataset == "fer2013":
    ...
    #以48*48的矩阵输入，48*48为像素值，因为是黑白，所以高度为1
    model = CNN3(input_shape=(48, 48, 1), n_classes=8)#分类器最后是八类
    #使用随机梯度下降算法，同时定义loss为交叉熵形式计算
    sgd = SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True)
    #sgd是梯度下降，optimizer是优化器，categorical_crossentropy交叉熵计算
    model.compile(optimizer=sgd, loss='categorical_crossentropy', metrics=['accuracy'])
    # 检查点，如果出错，就保存到./models/cnn2_best_weights.h5
    callback = [
    ...
    #开始训练，fit_generator训练模型，将小黑窗口输入的数据读到opt优化器中，opt.epochs小黑窗口输入的遍历次数
    #validation_data = valid_generator验证模型的准确性
